    ...
    <div className="flex flex-col lg:flex-row items-center justify-center py-8 overflow-x-hidden">
      ...
      <ul className="space-y-4 text-left lg:mr-32 w-full lg:w-auto ml-5">
        {menuItems.map((item, index) => (
          <li
            key={index}
            className="relative flex items-center cursor-pointer text-gray-800 font-medium text-base hover:text-[grey] transition-all duration-200"
            onClick={() =>
          ...
          >
            ...
            {item.options && openDropdown === index && (
              <ul className="absolute left-full top-0 mt-2 bg-white shadow-lg rounded-md p-2 w-48 z-10">
                {/* Close Option */}
                <li
                  className="flex items-center justify-end px-4 py-2 text-gray-800  hover:text-[grey] rounded-md transition-all duration-200 cursor-pointer"
